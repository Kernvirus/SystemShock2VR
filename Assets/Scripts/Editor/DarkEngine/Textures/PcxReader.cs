...
namespace Assets.Scripts.Editor.DarkEngine.Textures
{
    ...
    public static class PcxReader
    {
        ...
        public static Tuple<Texture2D, bool> Load(string fileName)
        {
            Tuple<Texture2D, bool> bmp = null;
        ...
        /// <returns>Bitmap that contains the image that was read.</returns>
        public static Tuple<Texture2D, bool> Load(Stream stream)
        {
            int imgWidth = -1;
            int imgHeight = -1;
            int imgBpp = -1;
            Texture2D theBitmap = null;
            ...
            if (imgWidth * imgHeight * 4 > bmpData.Length)
                Debug.Log("Not ");

            // fix transparency
            bool hasAlpha = false;
            for (int k = 0; k < bmpData.Length; k += 4)
            {
                if (bmpData[k + 2] == colorPalette[0] && bmpData[k + 1] == colorPalette[1] && bmpData[k] == colorPalette[2])
                {
                    bmpData[k + 3] = 0;
                    hasAlpha = true;
                }
                ...
                {
                    bmpData[k + 3] = byte.MaxValue;
                }
            }
            
            byte[] vFlipedData = new byte[bmpData.Length];
            for (int y2 = 0; y2 < imgHeight; y2++)
            {
                int lineOffset = bmpData.Length - (y2 + 1) * imgWidth * 4;
                int oppositeLineOffset = y2 * imgWidth * 4;

                for (int x2 = 0; x2 < imgWidth; x2++)
                {
                    vFlipedData[lineOffset + x2 * 4    ] = bmpData[oppositeLineOffset + x2 * 4];
                    vFlipedData[lineOffset + x2 * 4 + 1] = bmpData[oppositeLineOffset + x2 * 4 + 1];
                    vFlipedData[lineOffset + x2 * 4 + 2] = bmpData[oppositeLineOffset + x2 * 4 + 2];
                    vFlipedData[lineOffset + x2 * 4 + 3] = bmpData[oppositeLineOffset + x2 * 4 + 3];
                }
            }

            theBitmap = new Texture2D((int)imgWidth, (int)imgHeight, TextureFormat.BGRA32, false);

            theBitmap.LoadRawTextureData(vFlipedData);
            theBitmap.Apply();
            return new Tuple<Texture2D, bool>(theBitmap, hasAlpha);
        }
